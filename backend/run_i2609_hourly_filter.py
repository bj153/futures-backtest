...
print("🦞 i2609 铁矿石 近1月小时线方向回测 + 动量过滤")
print("=" * 60)
...
exclude = ['time', 'datetime', 'id', 'label_ret', 'label_dir', 'close', 'open', 'high', 'low',
           'volume', 'close_oi', 'open_oi', 'symbol', 'duration', 'ret_1', 'ma20']
feature_cols = [c for c in df.columns if c not in exclude and df[c].dtype in ('float64', 'int64')]

df = df.dropna(subset=feature_cols + ['label_dir', 'label_ret', 'ma20_direction']).reset_index(drop=True)
...
while i + TEST_SIZE <= len(df):
    ...
    for j in range(len(test)):
        results.append({
            ...
            'ma20_dir': test['ma20_direction'].iloc[j],
        })
...
print(f"📅 严格近1个月: {result_df['time'].min().strftime('%Y-%m-%d')} ~ {result_df['time'].max().strftime('%Y-%m-%d')} ({len(result_df)}根K线)")
...
for version, label in [(False, '原始'), (True, '+动量过滤')]:
    print(f"\n--- {label} ---")
    ...
    for th in [0.0, 0.55, 0.58, 0.60, 0.62, 0.65]:
        ...
        if version:
            # 动量过滤：趋势向下(ma20_dir=-1)时不做多，趋势向上(ma20_dir=1)时不做空
            mask = ~np.isnan(pred)
            if mask.sum() < 5:
                continue
            ...
            test_ret_vals = result_df['actual_ret'].values[final_mask]
        else:
            mask = ~np.isnan(pred)
            if mask.sum() < 5:
                continue
        ...
        longs = int((p == 1).sum())
        shorts = int((p == 0).sum())
        print(f"{th:>7.2f} {int(len(p)):>6} {longs:>5} {shorts:>5} {switches:>5} {total:>9.2%} {sharpe:>6.2f} {win:>5.2%} {dd:>7.2%}")
