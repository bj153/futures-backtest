"""批量回测 eagle_ladder_k：多品种 × 多周期，结果存 batch_results.json"""
import json, sys, urllib.request

strategy = open('backend/strategies/eagle_ladder_k.py', encoding='utf-8').read()
...
results = []
try:
    results = json.load(open('batch_results.json', encoding='utf-8'))
except Exception:
    pass
...
for code, mult in CONTRACTS.items():
    for freq in FREQS:
        ...
        json.dump(results, open('batch_results.json', 'w', encoding='utf-8'),
