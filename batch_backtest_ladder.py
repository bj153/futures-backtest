"""批量回测 ladder_double_k：多品种 × 多周期，结果存 batch_results_ladder.json"""
import json, sys, urllib.request

strategy = open('backend/strategies/ladder_double_k.py', encoding='utf-8').read()
...
results = []
try:
    results = json.load(open('batch_results_ladder.json', encoding='utf-8'))
except Exception:
    pass
...
for code, mult in CONTRACTS.items():
    for freq in FREQS:
        ...
        json.dump(results, open('batch_results_ladder.json', 'w', encoding='utf-8'),
